...
#os.environ["PATH"] += os.pathsep + str(r'C:/Program Files (x86)/Graphviz2.38/bin/').replace('/','\\')

eventlog = 'OneCDS_empowerlog_2019.csv'
file_path = os.path.join('..\data', eventlog)

df = pd.read_csv(file_path, encoding = 'utf-8')
df.rename(columns = {'CASE_ID': 'CaseID','Activity':'ActivityID','Timestamp':'CompleteTimestamp'}, inplace = True)
df['CompleteTimestamp'] = pd.to_datetime(df['CompleteTimestamp'])


#pre-model encode/decode functions
...
df.sort_values(['CaseID', 'CompleteTimestamp'], inplace=True, ignore_index=True)

# map activity name to number
act_to_int = dict((act, i+1)
                  for i, act in enumerate(list(df.ActivityID.unique())))
int_to_act = dict((i+1, act)
                  for i, act in enumerate(list(df.ActivityID.unique())))
df['ActivityID'] = df['ActivityID'].map(act_to_int)
...
df_subset.groupby("CaseID").count()["ActivityID"].hist(bins= 1000)
filter = df_subset.groupby("CaseID").count().ActivityID
list = filter[(filter>2) & (filter < 60) ].index.tolist()
df_subset = df_subset.loc[df_subset.CaseID.isin(list)].reset_index()
df_subset = df_subset.head(15000)
...
all = create_seq2(df_subset, 'ActivityID')
X = all[0]
...
l1 = LSTM(100, implementation=2, kernel_initializer='glorot_uniform', return_sequences=True, dropout=0.2, name='Layer_1')(main_input) # the shared layer
b1 = BatchNormalization(name='Normalize_1')(l1)
l2_1 = LSTM(100, implementation=2, kernel_initializer='glorot_uniform', return_sequences=False, dropout=0.2, name='Layer_2a')(b1) # the layer specialized in activity prediction
b2_1 = BatchNormalization(name='Normalize_2a')(l2_1)
l2_2 = LSTM(100, implementation=2, kernel_initializer='glorot_uniform', return_sequences=False, dropout=0.2, name='Layer_2b')(b1) # the layer specialized in time prediction
b2_2 = BatchNormalization(name='Normalize_2b')(l2_2)
...
plot_model(model, to_file='plot2.png', show_shapes= True, rankdir = 'LR')
